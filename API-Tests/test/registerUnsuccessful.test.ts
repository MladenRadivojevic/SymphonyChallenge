...
describe('Unsuccessful registraion', () => {
  it('Registration shoul be unsuccessful', async () => {
    ...
    try {
      await axios.post(
        ReqresApi.registerUnsuccessful,
        new RegisterDTO({ email: generatedEmail, password: generatedPassword })
      );
    } catch (e) {
      console.log(
        `Data used for testing: Email: ${generatedEmail}, Password: ${generatedPassword}`
      );
