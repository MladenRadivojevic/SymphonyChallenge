...
import * as get_list_of_users_schema from '../src/json/list-users/list-users-.json';
describe('List Users | Health check', () => {
  it('Should return list of users', async () => {
    const get_list_of_users: AxiosResponse = await axios.get(ReqresApi.listUsers);
...
describe('List Users | Response Validation', () => {
  it('Response should be valid', async () => {
    const get_list_of_users: AxiosResponse = await axios.get(ReqresApi.listUsers);
    const is_response_valid = responseValidation(get_list_of_users_schema, get_list_of_users);
